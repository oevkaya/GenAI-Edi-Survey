...
ChatGPT_Survey_raw <- read_csv("ChatGPT_Survey.csv")
Item_Names <- read_csv('Acroynms.csv')

ChatGPT_Survey <- ChatGPT_Survey_raw %>% 
  select(!c("1. Tick this box to agree to take part in the survey" , 
  "16. Please indicate to what extent you agree or disagree with each statement below." ,
  "18. Please indicate to what extent you agree or disagree with each statement below.",
  "20. Please indicate to what extent you agree or disagree with each statement below.",
  "22. If you would like to take part in the prize draw then please provide your university email address." ))

colnames(ChatGPT_Survey) <- Item_Names$Acroynms

# Drop other not necessary columns
ChatGPT_Survey <- ChatGPT_Survey %>% select(!c('Completion'))

# Extracting the open ended type questions
ChatGPT_Survey_open_ended <- ChatGPT_Survey %>% 
  select('Response_ID', 'Familiarity_open', 'Benefit_Challenge_open',
          'Future_open', 'GenAI_Not_Used', 'GenAI_Future_Usage', 'Add_Comments',
          'GenAI_Which', 'GenAI_Other_Ways')


# Drop other columns related to open-ended type of questions
ChatGPT_Survey <- ChatGPT_Survey %>% 
  select(!c('Familiarity_open', 'Benefit_Challenge_open',
            'Future_open', 'GenAI_Not_Used', 'GenAI_Future_Usage', 'Add_Comments',
            'GenAI_Which', 'GenAI_Other_Ways'))

# View the cleaned data
library(writexl)
write_xlsx(ChatGPT_Survey, "ChatGPT_Survey_Items.xlsx")
...
```{r}
# Changing the values for some columns for the simplicity

# Assuming ChatGPT_Survey is your data frame and UG_PGT is the column you want to modify
ChatGPT_Survey_new <- ChatGPT_Survey %>%
  mutate(UG_PGT = case_when(
    UG_PGT == "Undergraduate (e.g. studying for a BSc or MMath)" ~ "UG",
    UG_PGT == "Postgraduate (e.g. studying for an MSc)" ~ "PGT",
    TRUE ~ UG_PGT # Keeps original value if none of the above conditions are met
  ))
...
# For the Current_Degree_1 details 
ChatGPT_Survey_new <- ChatGPT_Survey_new %>%
  mutate(Current_Degree_1 = case_when(
    Current_Degree_1 == "Mathematics (MA Hons) (UTMATHM)" ~ "UTMATHM",
    Current_Degree_1 == "Mathematics (MMath Hons) (UTMMATHEMA1F)" ~ "UTMMATHEMA1F",
    Current_Degree_1 == "Other, please specify" ~ "Others",
    Current_Degree_1 == "Applied Mathematics (BSc Hons) (UTAPMAT)" ~ "UTAPMAT",
    Current_Degree_1 == "Mathematics and Statistics (BSc Hons) (UTMATST)" ~ "UTMATST",
    Current_Degree_1 == "Mathematics (BSc Hons) (UTMATHB)"  ~ "UTMATHB",
    Current_Degree_1 == "Applied Mathematics (MMath) (UTMMAAPPMA1F)"  ~ "UTMMAAPPMA1F",
    Current_Degree_1 == "Mathematics and Music (BSc Hons) (UTMATMU)"  ~ "UTMATMU",
    Current_Degree_1 == "Mathematics and Business (BSc Hons) (UTBSCMATBU1F)"  ~ "UTBSCMATBU1F",
    Current_Degree_1 == "Mathematics and Physics (BSc Hons) (UTMATPH)"  ~ "UTMATPH",
    Current_Degree_1 == "Mathematics and Biology (BSc Hons) (UTBSCMATBI1F)"  ~ "UTBSCMATBI1F",
    TRUE ~ Current_Degree_1 # Keeps original value if none of the above conditions are met
  ))
...
```{r}
# For the Current_Degree_2 details 
ChatGPT_Survey_new <- ChatGPT_Survey_new %>%
  mutate(Current_Degree_2 = case_when(
    Current_Degree_2 == "(PTMSCORCPO1F)Operational Research with Data Science (MSc) - 1 Year (PTMSCOPRDS1F)" ~ "PTMSCOPRDS1F",
    Current_Degree_2 == "Operational Research (MSc) (Full-time) (PTMSCORSCH1F)" ~ "PTMSCORSCH1F",
    Current_Degree_2 == "Computational Applied Mathematics (MSc) - 1 Year (Full-time) (PTMSCCOMAM1F)"  ~ "PTMSCCOMAM1F",
    Current_Degree_2 == "Statistics with Data Science (MSc) (PTMSCSTADS1F)" ~ "PTMSCSTADS1F",
    Current_Degree_2 == "Computational Mathematical Finance (MSc) (Full-time) (PTMSCCOMAF1F)" ~ "PTMSCCOMAF1F",
    Current_Degree_2 == "Other, please specify"  ~ "Others",
    TRUE ~ Current_Degree_2 # Keeps original value if none of the above conditions are met
  ))
...
# View the cleaned data
library(writexl)
write_xlsx(ChatGPT_Survey_new, "ChatGPT_Survey_Items.xlsx")
```
...
What about the details of Others degree programme? 
Note that this part needs further data pre-processing 
...
# Step 1: Categorize and Map similar programs
ChatGPT_Survey_others <- ChatGPT_Survey_new  %>%
  mutate(Category = case_when(
    str_detect(Degree_Prog_1, regex("Artificial Intelligence", ignore_case = TRUE)) ~ "AI",
    str_detect(Degree_Prog_1, regex("AI", ignore_case = TRUE)) ~ "AI",
    str_detect(Degree_Prog_1, regex("computer science", ignore_case = TRUE)) ~ "CS",
    str_detect(Degree_Prog_1, regex("chemistry", ignore_case = TRUE)) ~ "Chem",
    str_detect(Degree_Prog_1, regex("Economics", ignore_case = TRUE)) ~ "Econ",
    str_detect(Degree_Prog_1, regex("Engineering", ignore_case = TRUE)) ~ "Eng",
    str_detect(Degree_Prog_1, regex("Mathematical", ignore_case = TRUE)) ~ "Math",
    str_detect(Degree_Prog_1, regex("Mathematics", ignore_case = TRUE)) ~ "Math",
    TRUE ~ "Other"
  ))

# Step 2: Aggregate the counts
aggregated_data <- ChatGPT_Survey_others %>%
  group_by(Category) %>%
  summarise(Count = n()) %>% 
  mutate(Percentage = round(Count / sum(Count) * 100, 2)) %>% 
...
ggplot(aggregated_data, aes(x = Category, y = Count, fill = Category)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Count of Others Degree Programs", x = "Category", y = "Total Count") +
  geom_text(aes(label = paste0(round(Percentage, 1), "%", "(", round(Count, 1), ")")), position = position_stack(vjust = 0.5)) +
  theme_minimal()
...
```{r}
ChatGPT_Current_degree2 <- ChatGPT_Survey_new %>%
  group_by(Current_Degree_2) %>%
  summarise(Count = n()) %>%
  mutate(Percentage = round(Count / sum(Count) * 100, 2)) %>% 
  drop_na(Current_Degree_2) %>% 
  arrange(desc(Percentage))

# Plot
ggplot(data = ChatGPT_Current_degree2, aes(x = reorder(Current_Degree_2, -Percentage), y = Percentage, fill = Current_Degree_2)) +
  geom_bar(stat = "identity") +
  coord_flip() + labs(title = 'Which degree programme are you currently following (PGT)?', 
                      subtitle = 'Degree Level overview of the participants',
                      caption = 'Data Source: SoM GenAI Survey') + 
  theme_bw() + 
  theme(legend.position = 'none', axis.title.y=element_blank()) +
  geom_text(aes(label = paste0(round(Percentage, 1), "%", "(", round(Count, 1), ")")), position = position_stack(vjust = 0.5)) +
  scale_x_discrete(breaks = ChatGPT_Survey_new$Current_Degree_2) # Adjust this based on your preferred breakdown points
```
...
```{r}
ggplot(data = ChatGPT_Survey_new %>% drop_na(Year_level), aes(x=GenAI_Familiarity, fill = GenAI_Familiarity)) +
  geom_bar() +  facet_grid(vars(Year_level)) + coord_flip() + ylab('') + 
...
```{r}
ggplot(data = ChatGPT_Survey_new %>% drop_na(Honours), aes(x=GenAI_Familiarity, fill = GenAI_Familiarity)) +
  geom_bar() +  facet_grid(vars(Honours)) + coord_flip() + ylab('') + 
  theme_bw() + theme(legend.position = 'none')
```
...
phrase_data <- phrase_data %>%
  mutate(
    ...
    Percentage = (Count / dim(ChatGPT_Survey)[1]) * 100
  )
...
ChatGPT_Survey_new <- ChatGPT_Survey_new %>%
  ...
  drop_na(Year_level) #After dropping the missing info on year level
...
summary_data <- data_for_plot %>%
  group_by(Honours, Familiarity_Grouped) %>%
...
ggplot(data_for_plot, aes(x = Honours, y = (Familiarity_1), fill = proportion)) +
  geom_tile() +
...
ggplot(data_for_plot, aes(x = Honours, y = (Familiarity_2), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Familiarity_3), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Familiarity_4), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Familiarity_5), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Familiarity_6), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Familiarity_7), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
ggplot(data_for_plot, aes(x = Honours, y = (Familiarity_8), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_1) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# Summarize the counts for each Honours and Familiarity_Grouped category
summary_data <- data_for_plot %>%
  group_by(Honours, Benefit_Challenge_Grouped) %>%
  summarise(Total_Count = sum(n), .groups = 'drop')

# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_1), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_2) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_2), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_3) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_3), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_4) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_4), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_5) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_5), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_6) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_6), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_7) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_7), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_8) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_8), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Creating a summary table to calculate counts
data_for_plot <- ChatGPT_Survey_new %>%
  count(Honours, Benefit_Challenge_9) %>%
  group_by(Honours) %>%
  mutate(proportion = n / sum(n)) %>%
  ungroup() %>%
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Benefit_Challenge_9), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# Summarize the counts for each Honours and Familiarity_Grouped category
summary_data <- data_for_plot %>%
  group_by(Honours, Future_Grouped) %>%
  summarise(Total_Count = sum(n), .groups = 'drop')

# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Future_1), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Future_2), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Future_3), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Future_4), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Future_5), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
# # Creating a summary table to calculate counts
ggplot(data_for_plot, aes(x = Honours, y = (Future_6), fill = proportion)) +
  geom_tile() +
  geom_text(aes(label = paste0("(", n, ")", "\n", scales::percent(proportion, accuracy = 0.1))), color = "black", size = 3) +
  scale_fill_gradient2(low = "white", mid = "grey", high = "steelblue") +
...
```{r}
#unique(ChatGPT_Survey_open_ended$GenAI_Not_Used)[3]
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$GenAI_Not_Used)[4]))
#unique(ChatGPT_Survey_open_ended$GenAI_Not_Used)[4]
#unique(ChatGPT_Survey_open_ended$GenAI_Not_Used)[6]
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$GenAI_Not_Used)[9]))
#unique(ChatGPT_Survey_open_ended$GenAI_Not_Used)[9]
```
...
```{r}
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Familiarity_open)[2]))
#unique(ChatGPT_Survey_open_ended$Familiarity_open)[2]

cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Familiarity_open)[3]))
#unique(ChatGPT_Survey_open_ended$Familiarity_open)[3]

cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Familiarity_open)[4]))
#unique(ChatGPT_Survey_open_ended$Familiarity_open)[4]
```
...
```{r}
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Benefit_Challenge_open)[2]))
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Benefit_Challenge_open)[7]))
# unique(ChatGPT_Survey_open_ended$Benefit_Challenge_open)[7]
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Benefit_Challenge_open)[44]))
# unique(ChatGPT_Survey_open_ended$Benefit_Challenge_open)[44]
```
...
```{r}
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Future_open)[2]))
#unique(ChatGPT_Survey_open_ended$Future_open)[3]
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Future_open)[4]))
# unique(ChatGPT_Survey_open_ended$Future_open)[4]
cat(sprintf('"%s"\n', unique(ChatGPT_Survey_open_ended$Future_open)[41]))
# unique(ChatGPT_Survey_open_ended$Future_open)[41]
```
