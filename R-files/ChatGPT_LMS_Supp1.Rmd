...
UG_PGT_level <- ChatGPT_Survey_new %>%
  group_by(UG_PGT) %>%
  summarise(Count = n()) %>%
...
```{r}
Obs_year_level <- ChatGPT_Survey_new %>%
...
# Plot
ggplot(aes(x = reorder(as.factor(Year_level), -Percentage), y = Percentage, fill = as.factor(Year_level))) + 
  geom_bar(stat = "identity") +
  coord_flip() + labs(title = 'UG Year level for the participants', 
